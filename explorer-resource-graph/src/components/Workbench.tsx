...
interface Message {
  ...
  finding?: FindingData;
  codeDiff?: CodeDiff;
  confirm?: { label: string; cardTitle?: string; continuation: Message[]; confirmDetails?: Array<{ label: string; value: string }> };
  confirmed?: boolean;
}
...
interface FindingData {
  ...
  title: string;
  rows: Array<{ label: string; value: string; hi?: boolean; highlight?: boolean }>;
}
...
const SCENARIOS: Record<string, Scenario> = {
  ...
  terraform_upgrade: {
    sessionTitle: "Terraform Upgrade Campaign · hashicorp-demo",
    sessionDesc: "Staged operational campaign to upgrade Terraform CLI across all workspaces — with risk analysis, rollout waves, and approval checkpoints.",
    inspectorTab: "Overview",
    steps: [
      { label: "Review risk assessment", sublabel: "Confirm before proceeding", status: "active" },
      { label: "Review simulation plan", sublabel: "3-wave rollout", status: "pending" },
      { label: "Simulation outputs", sublabel: "All 8 workspaces", status: "pending" },
      { label: "Wave 1 execution", sublabel: "4 low-risk workspaces", status: "pending" },
      { label: "Wave 2 execution", sublabel: "3 medium-risk workspaces", status: "pending" },
      { label: "Wave 3 execution", sublabel: "2 high-risk workspaces", status: "pending" },
      { label: "Post-upgrade verification", status: "pending" },
    ],
    artifacts: [
      { name: "Risk assessment report", size: "8 MB" },
      { name: "Dependency map", size: "3 MB" },
      { name: "Compatibility analysis", size: "4 MB" },
    ],
    messages: [
      m("user", "Upgrade Terraform version across all workspaces"),

      // ── PHASE 1: Risk assessment ───────────────────────────────────────────────
      m("system", "Scan complete. Running full risk assessment — classifying workspaces by environment, dependency map, resource count, and provider compatibility.", "Scanned 24 workspaces · Mapping dependencies · Checking compatibility"),

      m("system", "Risk assessment complete.", "Risk analysis done · Dependency graph built · 1 advisory found", {
        finding: {
          status: "warn",
          title: "8 workspaces identified · 2 HIGH · 3 MEDIUM · 3 LOW",
          rows: [
            { label: "HIGH risk (2)", value: "payments-prod-* · production + advisory detected", hi: true },
            ...
            { label: "Dependency found", value: "payments-eu-west depends on networking-prod-core" },
            { label: "Compatibility advisory", value: "1 · payments-prod-us-east · deprecated syntax" },
            { label: "Estimated campaign duration", value: "45 – 60 minutes" },
          ],
        },
      }),

      m("system", "Full per-workspace breakdown:", "Cross-workspace dependencies mapped", {
        table: {
          caption: "Workspace risk assessment · why each workspace was selected",
          columns: ["Workspace", "Version", "Risk", "Reason"],
          rows: [
            ["payments-prod-us-east", "1.5.2", "HIGH", "Production · deprecated remote_state syntax"],
            ["payments-prod-eu-west", "1.5.2", "HIGH", "Production · depends on networking-prod-core"],
            ["networking-prod-core", "1.6.3", "MEDIUM", "Core infra · 61 resources · shared by 3 workspaces"],
            ["api-gateway-prod", "1.6.3", "MEDIUM", "Shared API layer · 22 resources"],
            ["data-pipeline-prod", "1.6.1", "MEDIUM", "Log format change in 1.7.x · advisory"],
            ["auth-service-prod", "1.6.7", "LOW", "Isolated · 29 resources · no dependencies"],
            ["cdn-global-prod", "1.6.0", "LOW", "Read-only CDN config · 12 resources"],
            ["inventory-service-staging", "1.5.7", "LOW", "Non-production · 18 resources"],
          ],
          selectedRow: 0,
        },
      }),
      ...
      m("system", "Review the risk classification above. Confirm to proceed to the simulation plan.", undefined, {
        finding: {
          status: "warn",
          title: "Advisory: deprecated terraform_remote_state syntax · payments-prod-us-east",
          rows: [
            { label: "Issue", value: "remote_state backend attribute format deprecated in 1.7.x", hi: true },
            { label: "Impact", value: "Warning during plan · not blocking · no data loss risk" },
            { label: "Recommended fix", value: "Update config syntax before Wave 3 executes" },
            { label: "Rollback available", value: "Yes — revert version pin at any time" },
          ],
        },
        confirm: {
          cardTitle: "Confirm risk assessment · proceed to simulation plan",
          label: "Risk assessment confirmed · Build simulation plan",
          continuation: [
            m("user", "Risk assessment confirmed"),

            // ── PHASE 2: Simulation plan ─────────────────────────────────────────
            m("system", "Generating 3-wave simulation plan based on risk levels and workspace dependencies.", "Building rollout waves · Ordering by dependency graph"),

            m("system", "Simulation plan ready. Review the 3-wave rollout before running the simulation.", "Rollout plan generated · Rollback scripts ready", {
              finding: {
                status: "info",
                title: "3-wave simulation plan · approval checkpoint before each wave",
                rows: [
                  { label: "Wave 1 · LOW risk (4)", value: "auth-service-prod, cdn-global-prod, inventory-staging, data-pipeline-prod" },
                  { label: "Wave 2 · MEDIUM risk (2)", value: "networking-prod-core, api-gateway-prod" },
                  { label: "Wave 3 · HIGH risk (2)", value: "payments-prod-us-east, payments-prod-eu-west" },
                  { label: "Approval checkpoint", value: "Required before each wave · pause at any point", hi: true },
                  { label: "Rollback plan", value: "Automatic on failure · manual override available" },
                  { label: "Sequencing rule", value: "Wave 2 blocked until Wave 1 health checks pass" },
                ],
              },
              confirm: {
                cardTitle: "Confirm simulation plan · run full dry-run",
                label: "Simulation plan confirmed · Run simulation",
                continuation: [
                  m("user", "Simulation plan confirmed"),

                  // ── PHASE 3: Simulation outputs ────────────────────────────────
                  m("system", "Running full simulation across all 8 workspaces. No runs will be triggered — this is a dry-run analysis only.", "Simulating all waves · Checking provider compatibility · Estimating plan deltas"),

                  m("system", "Simulation complete. 7 of 8 workspaces are fully safe. 1 advisory in Wave 3.", "Simulation passed · 0 breaking changes · 1 advisory", {
                    table: {
                      caption: "Simulation outputs · all 8 workspaces · Terraform 1.5.x / 1.6.x → 1.7.4",
                      columns: ["Workspace", "Wave", "Result", "Notes"],
                      rows: [
                        ["auth-service-prod",       "W1", "✓ Safe",    "0 breaking changes · 29 resources"],
                        ["cdn-global-prod",          "W1", "✓ Safe",    "0 breaking changes · 12 resources"],
                        ["inventory-staging",        "W1", "✓ Safe",    "0 breaking changes · 18 resources"],
                        ["data-pipeline-prod",       "W1", "⚠ Advisory","Log format change · not blocking"],
                        ["networking-prod-core",     "W2", "✓ Safe",    "61 resources · no breaking changes"],
                        ["api-gateway-prod",         "W2", "✓ Safe",    "22 resources · no breaking changes"],
                        ["payments-prod-us-east",    "W3", "⚠ Advisory","remote_state syntax warning"],
                        ["payments-prod-eu-west",    "W3", "✓ Safe",    "Blocked until Wave 2 completes"],
                      ],
                      selectedRow: 0,
                    },
                    confirm: {
                      cardTitle: "Approve Wave 1 execution · 4 workspaces",
                      label: "Approve Wave 1 · Begin upgrades",
                      continuation: [
                        m("user", "Wave 1 approved"),
                        m("system", "Wave 1 initiated. Terraform 1.7.4 pinned in all 4 workspace settings. Upgrades apply on each workspace's next triggered run.", "Pinning 1.7.4 · Notifying workspace owners"),
                        m("system", "Wave 1 complete. All 4 workspaces upgraded and verified healthy. Ready for Wave 2 when you are.", "Post-upgrade verification passed · Health checks complete", {
                          finding: {
                            status: "ok",
                            title: "Wave 1 complete · 4/4 workspaces on Terraform 1.7.4",
                            rows: [
                              { label: "auth-service-prod", value: "1.7.4 · Applied · Healthy" },
                              { label: "cdn-global-prod", value: "1.7.4 · Applied · Healthy" },
                              { label: "inventory-staging", value: "1.7.4 · Applied · Healthy" },
                              { label: "data-pipeline-prod", value: "1.7.4 · Applied · Log advisory suppressed" },
                              { label: "Rollback", value: "Not required" },
                              { label: "Next", value: "Wave 2 · networking-prod-core + api-gateway-prod · ready", hi: true },
                            ],
                          },
                        }),
                      ],
...
SCENARIOS.provider_upgrade_failure = {
  ...
  steps: [
    ...
    { label: "Confirm target workspaces",                                               status: "active" },
  ...
  messages: [
    ...
    m("system", "Select the workspaces to remediate. Both failing workspaces are pre-selected based on the assessment above.", "2 affected workspaces identified · Pre-selected for remediation", {
      table: {
        caption: "Confirm target workspaces · aws provider v5 remediation",
        columns: ["Workspace", "Provider", "Status", "Risk"],
        rows: [
          ["payments-prod-us-east", "aws ~> 5.0", "FAILING", "CRITICAL"],
          ["networking-prod-core",  "aws ~> 5.0", "FAILING", "CRITICAL"],
      ...
      confirm: {
        cardTitle: "Confirm target workspaces · aws provider v5 remediation",
        label: "Confirm workspaces — proceed to remediation plan",
        continuation: [
          m("user", "Both workspaces confirmed — generate remediation plan"),
          ...
          m("system", "PR #247 raised. A Jira ticket has been pre-filled with the change details — review and confirm to log it before proceeding to merge.", "PR pushed to GitHub · Jira pre-filled from PR metadata", {
            confirm: {
              ...
              confirmDetails: [
                ...
                { label: "Reporter",    value: "Terraform Operator (automated)" },
              ],
...
function MetaLine({ text }: { text: string }) {
  return (
    <div className="flex items-center gap-1.5 px-6 mb-1">
      <span style={{ color: M.metaColor, fontSize: "12px", fontFamily: M.font }}>Queried 1 datasource</span>
      <span style={{ color: M.metaColor, fontSize: "12px" }}>·</span>
      <span style={{ color: M.textDark, fontSize: "12px", fontFamily: M.font, textDecoration: "underline", cursor: "pointer" }}>View thinking</span>
    </div>
  );
}
...
// ── Explorer-specific SCENARIOS ────────────────────────────────────────────────
// Added to SCENARIOS inline via Object.assign after initial declaration
Object.assign(SCENARIOS, {
  blast_radius: {
    sessionTitle: "Blast Radius · networking-prod-core",
    sessionDesc: "Dependency traversal and blast-radius assessment for networking-prod-core across 6 downstream workspaces.",
    inspectorTab: "Infragraph" as const,
    steps: [
      { label: "Query resource catalog",               sublabel: "networking-prod-core",    status: "done"    as const },
      { label: "Identify downstream consumers",        sublabel: "6 workspaces found",      status: "done"    as const },
      { label: "Assess blast-radius hops",             sublabel: "2 hop levels",            status: "active"  as const },
      { label: "Confirm scope",                        status: "pending" as const },
      { label: "Prepare remediation packet",           status: "pending" as const },
    ],
    artifacts: [
      { name: "Blast-radius report",        size: "12 MB" },
      { name: "Affected workspace list",    size: "4 MB"  },
      { name: "Remediation packet draft",   size: "6 MB"  },
    ],
    messages: [
      m("user",   "Show blast radius of networking-prod-core"),
      m("system", "Querying resource catalog for networking-prod-core...", "Checked 8 workspaces · 341 resources indexed", {}),
      m("system", "**Blast-radius candidate identified.**", undefined, {
        finding: {
          status: "failed",
          title: "networking-prod-core · High blast radius",
          rows: [
            { label: "Drifted resources", value: "3 (vpc, subnet, nat-gw)", highlight: true },
            { label: "Hop-1 consumers",   value: "5 workspaces",            highlight: true },
            { label: "Hop-2 consumers",   value: "1 workspace (cdn-global)"                 },
            { label: "Total exposed",     value: "6 workspaces · 180 resources"             },
          ],
        },
      }),
      m("system", "Affected workspaces by hop distance:", undefined, {
        table: {
          caption: "Blast radius — workspace impact",
          columns: ["Workspace", "Hop", "Resources", "Risk"],
          rows: [
            ["payments-prod-us-east", "Hop 1", "47", "HIGH"],
            ["payments-prod-eu-west", "Hop 1", "43", "HIGH"],
            ["api-gateway-prod",      "Hop 1", "22", "MEDIUM"],
            ["auth-service-prod",     "Hop 1", "29", "MEDIUM"],
            ["data-pipeline-prod",    "Hop 1", "35", "LOW"],
            ["cdn-global-prod",       "Hop 2", "12", "LOW"],
          ],
          selectedRow: 0,
        },
      }),
      m("system", "Ready to prepare remediation packet. Confirm scope to proceed.", undefined, {
        confirm: {
          label: "Confirm scope — prepare remediation packet",
          continuation: [
            m("system", "Remediation packet prepared for networking-prod-core blast radius.", undefined, {
              finding: {
                status: "ok",
                title: "Remediation packet ready",
                rows: [
                  { label: "Blast-radius report",     value: "12 MB · ready for download"  },
                  { label: "Affected workspace list",  value: "6 workspaces · 180 resources" },
                  { label: "Recommended action",      value: "Stage changes — Wave 1: low-risk workspaces first", highlight: true },
                ],
              },
            }),
          ],
  ...
  dependency_traversal: {
    sessionTitle: "Dependency Traversal · aws_vpc.main",
    sessionDesc: "Trace resource dependencies within and across workspaces for aws_vpc.main in networking-prod-core.",
    inspectorTab: "Infragraph" as const,
    steps: [
      { label: "Locate resource",            sublabel: "aws_vpc.main",           status: "done"    as const },
      { label: "Trace dependencies",         sublabel: "5 dependents found",     status: "done"    as const },
      { label: "Map transitive consumers",   status: "active"  as const },
      { label: "Export dependency report",   status: "pending" as const },
    ],
    artifacts: [
      { name: "Dependency graph export", size: "3 MB" },
      { name: "Dependency report",       size: "2 MB" },
    ],
    messages: [
      m("user",   "What depends on aws_vpc.main in networking-prod-core"),
      m("system", "Tracing dependencies for aws_vpc.main...", "Checked 341 resources · 2 workspaces", {}),
      m("system", "**5 resources depend on aws_vpc.main.**", undefined, {
        finding: {
          status: "warn",
          title: "aws_vpc.main · 5 dependents · 2 workspaces",
          rows: [
            { label: "Explicit depends_on",    value: "2 resources",    highlight: true },
            { label: "Implicit (vpc_id attr)", value: "2 resources",    highlight: false },
            { label: "Implicit (subnet_id)",   value: "1 resource",     highlight: false },
            { label: "Cross-workspace",        value: "3 resources in payments-prod-us-east", highlight: true },
          ],
        },
      }),
      m("system", "Dependency chain:", undefined, {
        table: {
          caption: "Dependents of aws_vpc.main",
          columns: ["Resource", "Workspace", "Reason"],
          rows: [
            ["aws_instance.app_server[0]", "payments-prod-us-east", "explicit"],
            ["aws_security_group.web_sg",  "payments-prod-us-east", "implicit:vpc_id"],
            ["aws_api_gateway_rest_api.main", "api-gateway-prod",   "explicit"],
            ["aws_subnet.private_a",       "networking-prod-core",  "implicit:vpc_id"],
            ["aws_nat_gateway.az_b",       "networking-prod-core",  "implicit:subnet_id"],
          ],
          selectedRow: 0,
        },
      }),
      m("system", "Dependency report ready.", undefined, {
        finding: {
          status: "ok",
          title: "Traversal complete",
          rows: [
            { label: "Report",   value: "Dependency graph export · 3 MB" },
            { label: "Next",     value: "Show these resources in Explorer table", highlight: true },
          ],
        },
      }),
    ],
...
function getGraphData(title: string): GraphData {
  ...
  // Blast radius
  if (title.includes("Blast Radius")) {
    return {
      title: "Blast radius · networking-prod-core · 6 affected",
      nodes: [
        { id: "src",  label: "networking-prod-core",   sub: "Blast source",              type: "policy",    x: 300, y: 40, highlight: true },
        { id: "h1a",  label: "payments-prod-us-east",  sub: "Hop 1 · CRITICAL",          type: "workspace", x: 80,  y: 150, failed: true },
        { id: "h1b",  label: "payments-prod-eu-west",  sub: "Hop 1 · CRITICAL",          type: "workspace", x: 230, y: 150, failed: true },
        { id: "h1c",  label: "api-gateway-prod",       sub: "Hop 1 · MEDIUM",            type: "workspace", x: 370, y: 150 },
        { id: "h1d",  label: "auth-service-prod",      sub: "Hop 1 · MEDIUM",            type: "workspace", x: 510, y: 150 },
        { id: "h1e",  label: "data-pipeline-prod",     sub: "Hop 1 · LOW",               type: "ec2",       x: 300, y: 240 },
        { id: "h2a",  label: "cdn-global-prod",        sub: "Hop 2 · LOW",               type: "network",   x: 300, y: 340 },
      ],
      edges: [
        { from: "src", to: "h1a", failed: true, highlight: true },
        { from: "src", to: "h1b", failed: true, highlight: true },
        { from: "src", to: "h1c", highlight: true },
        { from: "src", to: "h1d", highlight: true },
        { from: "src", to: "h1e", highlight: true },
        { from: "h1e", to: "h2a" },
      ],
    };
  }
...
function SessionSidebar({ currentTitle, currentDesc, started }: { currentTitle: string; currentDesc: string; started: boolean }) {
  return (
    <div className="flex flex-col h-full" style={{ backgroundColor: "white" }}>
      {/* Search */}
      <div className="px-5 py-4">
        <div className="flex items-center px-3" style={{ backgroundColor: "#f7f8fa", borderRadius: "8px", height: "40px" }}>
          <input
...
const CONTEXT_HINTS: Record<string, { intro: string; examples: string[] }> = {
  ...
  explorer: {
    intro: "You're in the Explorer Resource Graph. Ask about resources, dependencies, blast radius, or provider versions across your entire Terraform estate.",
...
const LANDING_WORKFLOWS = [
  ...
  // Explorer-specific workflows
  {
    id: "blast_radius_analysis",
    label: "Blast-Radius Analysis",
    desc: "Scan for high-blast-radius workspaces in the resource catalog, analyze cross-workspace dependency hops, and prepare a remediation packet.",
    query: "Show blast radius of networking-prod-core",
    color: "#EF4444",
    tags: ["Explorer", "Dependency"],
    iconPath: "M13 10V3L4 14h7v7l9-11h-7z",
  },
  {
    id: "dependency_traversal",
    label: "Dependency Traversal",
    desc: "Trace resource dependencies within and across workspaces. Identify explicit and implicit attribute references for any resource.",
    query: "What depends on aws_vpc.main in networking-prod-core",
    color: "#7C3AED",
    tags: ["Explorer", "Read-only"],
    iconPath: "M9 17V7m0 10a2 2 0 01-2 2H5a2 2 0 01-2-2V7a2 2 0 012-2h2a2 2 0 012 2m0 10a2 2 0 002 2h2a2 2 0 002-2M9 7a2 2 0 012-2h2a2 2 0 012 2m0 0v10",
  },
];
    ...
    <div className="flex flex-col" style={{ fontFamily: M.font, width: "100%", height: "100%", overflow: "hidden" }}>
      ...
      <div className="flex items-center justify-between flex-shrink-0 px-4" style={{ height: "48px", backgroundColor: M.dark, borderBottom: `1px solid ${M.darkBorder}` }}>
        <div className="flex items-center gap-2.5">
          ...
          <span style={{ color: M.textWhite, fontSize: "13px", fontWeight: 600 }}>Terraform Operator</span>
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={onClose} className="flex items-center gap-1.5 px-3 py-1.5 rounded" style={{ color: M.textDimWhite, fontSize: "12px", backgroundColor: M.darkItem, border: `1px solid ${M.darkBorder}`, fontWeight: 500, cursor: "pointer" }}>
      ...
      <div className="flex flex-1 min-h-0">
        {/* ── Session sidebar (no icon rail) ── */}
        <div className="flex-shrink-0" style={{ width: "260px", borderRight: `1px solid ${M.rightBorder}` }}>
          <SessionSidebar currentTitle={scenario.sessionTitle} currentDesc={scenario.sessionDesc} started={started} />
        </div>
        ...
        {/* ── Right panel: tabbed inspector (equal width to chat) ── */}
